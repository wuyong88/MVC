...
//   $f_id= $_POST['f_id'];
//     $res = insert("insert into news(name,f_id) 
//     	values('$cate_name','$f_id')");  

//     if (!$res) {
//     	echo '<script>alert("失败");window.location.href="index.php?m=1&c='.C.'&a=add"</script>';
//     	exit;
//     }
...
// 判断用户是否提交
  if($_POST){
    ...
    $pid = $_POST['pid'];
    $res = insert("insert into news_cates(
           name,
           pid
  )values(
           '$name',
    ...
    if(!$res){
      echo '<script>alert("失败");window.location.href="index.php?m=1&c=news_cates&a=add"</script>';

    }
    echo '<script>alert("成功");window.location.href="index.php?m=1&c=news_cates&a=index"</script>';
...
require 'app/view/admin/news_cates/add.html';
